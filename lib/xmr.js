"use strict";
const multiHashing = require('wazn-hashing');
const cnUtil = require('wazn-utilities');
...
function detectAlgo(default_pool_algo_set, block_version) {
    if ("cn-wazn" in default_pool_algo_set) return block_version >= 11 ? "cn-wazn";
...
function processShare(miner, job, blockTemplate, nonce, resultHash) {
    ...
    if (hashDiff.ge(blockTemplate.targetDiff)) {
        ...
        if (++ verified_share_num <= MAX_VER_SHARES_PER_SEC*VER_SHARES_PERIOD) {
            ...
            if (cnAlgorithm=="cryptonight_wazn" && hard_fork_version>=11) {
                hash = multiHashing.cryptonight_wazn(convertedBlob);
            } else {
