...
cd ~
git clone https://github.com/project-wazn/wazn-proxy
...
cd ~
pm2 status
sudo env PATH=$PATH:`pwd`/.config/nvm/versions/node/v11.15.0/bin `pwd`/.config/nvm/versions/node/v11.15.0/lib/node_modules/pm2/bin/pm2 startup systemd -u $CURUSER --hp `pwd`
