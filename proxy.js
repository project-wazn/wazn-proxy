...
global.threadName = '';
var nonceCheck = new RegExp("^[0-9a-f]{8}$");
var activePorts = [];
...
var defaultPools = {};
var accessControl = {};
var lastAccessControlLoadTime = null;
var masterStats = {shares: 0, blocks: 0, hashes: 0};
    ...
    switch (message.type) {
        case 'newBlockTemplate':
            if (message.host in activePools){
                ...
                for (var miner in activeMiners){
                    if (activeMiners.hasOwnProperty(miner)){
                        var realMiner = activeMiners[miner];
...
// Combined Functions
function readConfig() {
    var local_conf = JSON.parse(fs.readFileSync('config.json'));
    if (typeof global.config === 'undefined') {
        global.config = {};
    }
    for (var key in local_conf) {
...
function Pool(poolData){
    ...
    this.algos            = {};
    for (var i in algo_arr) this.algos[algo_arr[i]] = this.default_algo_set[algo_arr[i]] = 1;
    ...
    this.sendShare = function (worker, shareData) {
        ...
        var job = this.poolJobs[worker.id].toarray().filter(function (job) {
    ...
    if (global.config.developerShare > 0){
        for (var pool in activePools){
            if (activePools.hasOwnProperty(pool)){
                ...
                var devPool = activePools[pool].coinFuncs.devPool;
...
function balanceWorkers(){
    ...
    var minerStates = {};
    poolStates = {};
    for (var poolName in activePools){
        if (activePools.hasOwnProperty(poolName)){
            var pool = activePools[poolName];
        ...
        if(poolStates.hasOwnProperty(coin)){
            if (poolStates[coin].totalPercentage !== 100){
                ...
                if (poolStates[coin].totalPercentage) {
                    var percentModifier = 100 / poolStates[coin].totalPercentage;
                    for (var pool in poolStates[coin]){
                    ...
                    var addModifier = 100 / poolStates[coin].activePoolCount;
                    for (var pool in poolStates[coin]){
                        if (poolStates[coin].hasOwnProperty(pool) && activePools.hasOwnProperty(pool)){
                            if (poolStates[coin][pool].devPool || !is_active_pool(pool)) continue;
    ...
    for (var workerID in activeWorkers){
        if (activeWorkers.hasOwnProperty(workerID)){
            for (var minerID in activeWorkers[workerID]){
                if (activeWorkers[workerID].hasOwnProperty(minerID)){
                    var miner = activeWorkers[workerID][minerID];
                    try {
                        var minerCoin = miner.coin;
    ...
    for (var coin in poolStates){
        if (poolStates.hasOwnProperty(coin) && minerStates.hasOwnProperty(coin)){
            var coinMiners = minerStates[coin];
            var coinPools = poolStates[coin];
            var devPool = coinPools.devPool;
            var highPools = {};
            var lowPools = {};
            delete(coinPools.devPool);
            if (devPool){
                var devHashrate = Math.floor(coinMiners.hashrate * (global.config.developerShare/100));
            ...
            if (Object.keys(highPools).length > 0){
                for (var pool in highPools){
                    if (highPools.hasOwnProperty(pool)){
                        for (var miner in coinPools[pool].miners){
                ...
                for (var pool in lowPools){
                    if (lowPools.hasOwnProperty(pool)){
                        ...
                        if(lowPools[pool] > 100){
                            for (var donatorPool in coinPools){
                                if(coinPools.hasOwnProperty(donatorPool) && !lowPools.hasOwnProperty(donatorPool)){
                                    for (var miner in coinPools[donatorPool].miners){
...
function enumerateWorkerStats() {
    var stats, global_stats = {miners: 0, hashes: 0, hashRate: 0, diff: 0};
    var pool_algos = {};
    var pool_algos_perf = {};
    for (var poolID in activeWorkers){
        if (activeWorkers.hasOwnProperty(poolID)){
            ...
            var inactivityDeadline = (typeof global.config.minerInactivityTime === 'undefined') ? Math.floor((Date.now())/1000) - 120
                : (global.config.minerInactivityTime <= 0 ? 0 : Math.floor((Date.now())/1000) - global.config.minerInactivityTime);
            for (var workerID in activeWorkers[poolID]){
                if (activeWorkers[poolID].hasOwnProperty(workerID)) {
                    var workerData = activeWorkers[poolID][workerID];
                    if (typeof workerData !== 'undefined') {
                        try{
                            ...
                            var miner_algos = workerData.algos;
			        ...
			        for (var algo in pool_algos[workerData.pool]) {
			           if (!(algo in miner_algos)) delete pool_algos[workerData.pool][algo];
                            ...
                            if (workerData.algos_perf) { // only process smart miners and add algo_perf from all smart miners
                                for (var algo in workerData.algos_perf) {
    ...
    for (var pool in pool_algos) {
        var pool_algos_perf2 = pool_algos_perf[pool];
...
function handlePoolMessage(jsonData, hostname){
    ...
    if (jsonData.hasOwnProperty('method')){
        ...
        var sendLog = pool.sendLog[jsonData.id];
...
function is_active_pool(hostname) {
    ...
    for (var poolName in activePools){
        ...
        var pool2 = activePools[poolName];
...
function Miner(id, params, ip, pushMessage, portData, minerSocket) {
    ...
    if (params.algo && (params.algo instanceof Array)) { // To report union of defined algo set to the pool for all its miners
        for (var i in params.algo) {
            this.algos = {};
            for (var i in params.algo) this.algos[params.algo[i]] = 1;
        }
    }
    ...
    var diffSplit = this.login.split("+");
...
/*
    if (this.algos) for (var algo in activePools[this.pool].default_algo_set) {
...
*/
    ...
    var pass_split = params.pass.split(":");
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage, minerSocket) {
     ...
     */
    var miner = activeMiners[params.id];
    ...
    switch (method) {
        case 'login':
            ...
            if (global.config.keepOfflineMiners) {
                for (var miner_id in activeMiners) {
                    if (activeMiners.hasOwnProperty(miner_id)) {
                        var realMiner = activeMiners[miner_id];
        ...
        case 'submit':
            ...
            var job = miner.validJobs.toarray().filter(function (job) {
            ...
            var activeBlockTemplate = activePools[miner.pool].activeBlocktemplate;
            var pastBlockTemplates = activePools[miner.pool].pastBlockTemplates;

            var blockTemplate = activeBlockTemplate.id === job.templateID ? activeBlockTemplate : pastBlockTemplates.toarray().filter(function (t) {
            ...
            var shareAccepted = miner.coinFuncs.processShare(miner, job, blockTemplate, params.nonce, params.result);
...
function activateHTTP() {
	var jsonServer = http.createServer((req, res) => {
		...
		if (req.url == "/") {
			var totalWorkers = 0, totalHashrate = 0;
			var poolHashrate = [];
		        var miners = {};
		        var offline_miners = {};
			var miner_names = {};
    			...
    			for (var offline_miner_id in offline_miners) {
			...
			var tablePool = "";
			var tableBody = "";
    			for (var miner_id in miners) {
				const miner = miners[miner_id];
				const name = miner.logString;
				var avgSpeed = miner.active ? miner.avgSpeed + " H/s" : "offline";
				var agent_parts = miner.agent.split(" ");
				tableBody += `
				<tr>
    			...
    			for (var poolName in poolHashrate) {
				var poolPercentage = (100*poolHashrate[poolName]/totalHashrate).toFixed(2);
				var targetDiff = activePools[poolName].activeBlocktemplate ? activePools[poolName].activeBlocktemplate.targetDiff : "?";
                		var walletId = activePools[poolName].username
				if (poolName.includes("moneroocean")) {
					var algo_variant = "";
                                        ...
                                        if (algo_variant != "") algo_variant = " (" + algo_variant + ")";
					tablePool += `<a class="${global.config.theme}" href="https://hive.wazniya.com/#worker_stats?addr=${walletId}" title="WAZNIYA Hive Dashboard" target="_blank"><h2> ${poolName}: ${poolHashrate[poolName]} H/s or ${poolPercentage}% (${targetDiff} diff) ${algo_variant}</h2></a>`;
				} else {
...
function activatePorts() {
    ...
    async.each(global.config.listeningPorts, function (portData) {
        ...
        var handleMessage = function (socket, jsonData, pushMessage, minerSocket) {
            if (!jsonData.id) {
	    ...
	    server.listen(portData.port, global.config.bindAddress, function (error) {
            ...
            var server = net.createServer(socketConn);
	    server.listen(portData.port, global.config.bindAddress, function (error) {
                if (error) {
                    console.error(global.threadName + "Unable to start server on: " + portData.port + " Message: " + error);
                    return;
...
function checkActivePools() {
    for (var badPool in activePools){
...
if (cluster.isMaster) {
    ...
    */
    ...
    setInterval(function(){
        for (var minerID in activeMiners){
